...
/// </summary>
public interface IFluentDataLoader<out TEntity> where TEntity : class
{
    /// <summary>
    ...
    IFluentDataLoader<TProperty> Load<TProperty>(Expression propertyPath) where TProperty : class;
    ...
    Task<IFluentDataLoader<TProperty>> LoadAsync<TProperty>(
        Expression propertyPath,
        CancellationToken cancellationToken = default (CancellationToken))
        where TProperty : class;

    /// <summary>Není určeno pro přímé volání. Interní použití.</summary>
    [EditorBrowsable(EditorBrowsableState.Advanced)]
    IFluentDataLoader<TWrappedEntity> Unwrap<TWrappedEntity>() where TWrappedEntity : class;
}