...
namespace underware.Data.Patterns.DataLoaders;
  ...
  {
    /// <summary>Načte vlastnosti objektů, pokud ještě nejsou načteny.</summary>
    ...
    IFluentDataLoader<TProperty> Load<TEntity, TProperty>(
      TEntity entity,
    ...
    Task<IFluentDataLoader<TProperty>> LoadAsync<TEntity, TProperty>(
      TEntity entity,
      Expression<Func<TEntity, TProperty>> propertyPath,
    ...
    void Load<TEntity>(
      TEntity entity,
      params Expression<Func<TEntity, object>>[] propertyPaths)
      where TEntity : class;
    ...
    /// <param name="propertyPath">Vlastnost, která má být načtena.</param>
    IFluentDataLoader<TProperty> LoadAll<TEntity, TProperty>(
    ...
    /// <param name="cancellationToken">Propagates notification that operations should be canceled.</param>
    Task<IFluentDataLoader<TProperty>> LoadAllAsync<TEntity, TProperty>(
      IEnumerable<TEntity> entities,
      Expression<Func<TEntity, TProperty>> propertyPath,
      CancellationToken cancellationToken = default (CancellationToken))
      where TEntity : class
